...
do_not_update_comm_after_n_days = 600
...
for item in data:
	...
	#  and if video was released less than `do_not_update_comm_after_n_days` days ago
	metadata_path = path+"/metadata.json"
	retrieve_metadata = True
	if os.path.exists(metadata_path):
		if os.stat(metadata_path).st_mtime > time.time() - update_comm_every_n_days*24*3600:
			retrieve_metadata = False
		else:
			with open(metadata_path, 'r') as f:
				metadata = json.load(f)
			released_data = datetime.datetime.strptime(metadata['info']['publishedAt'][:10], "%Y-%m-%d")
		if (datetime.datetime.utcnow() - released_data).days > do_not_update_comm_after_n_days:
			retrieve_metadata = False
